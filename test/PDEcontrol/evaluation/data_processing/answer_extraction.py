...
def extract_pdecontrol_few_shot_answer(input, reasoning, task):
    """
    In the case of pdecontrol, the answer is the whole generated python code.

    This is not the function for removing model halucination, that should
    have been done already in the main script.
    """
    return reasoning.strip()
    # if '### Instruction:' in reasoning:
    #     reasoning = reasoning.split("### Instruction:", 1)[0]
    # patt = regex.search(r"### Response:\s*(?P<ans>.*)", reasoning)
    # if patt is None:
    #     pred = "[invalid]"
    #     print(f"DEBUG >>>\n{reasoning}", flush=True)
    # else:
...
"""\nSpatial Signal Temporal Logic:\n```latex\n\\[\\phi = \\left( \\mathbf{G}_{[0.100000, 0.300000]} \\left( \\forall x \\in [50000, 70000] \\; \\left( u(x) - \\left( 0.000020 \\cdot x + -1.100000 \\right) > 0 \\right) \\right) \\land \\mathbf{G}_{[0.300000, 0.400000]} \\left( \\forall x \\in [50000, 70000] \\; \\left( u(x) - \\left( 0.000020 \\cdot x + -1.100000 \\right) < 0 \\right) \\right) \\right) \\lor \\left( \\mathbf{G}_{[0.420000, 0.500000]} \\left( \\forall x \\in [50000, 70000] \\; \\left( u(x) - \\left( -0.000020 \\cdot x + 1.500000 \\right) < 0 \\right) \\right) \\land \\mathbf{G}_{[0.500000, 0.600000]} \\left( \\forall x \\in [50000, 70000] \\; \\left( u(x) - \\left( 0.000020 \\cdot x + -1.100000 \\right) > 0 \\right) \\right) \\right)\\]\n```\n\nPython:\n```python\n\nfrom femformal.core.fem import mechlinfem as mechlinfem\n\n\nN = 20\nL = 100000\nrho = lambda x: 8e-06 if x < 30000 or x > 60000 else 8.5e-06\nE = lambda x: 200000000.0 if x < 30000 or x > 60000 else 100000000.0\nxpart = np.linspace(0, L, N + 1)\ng = [0.0, None]\nf_nodal = np.zeros(N + 1)\ndt = .0025\n\nT = 0.6\nsosys = mechlinfem.mechlinfem(xpart, r"""

def extract_pdecontrol_python_answer(input, reasoning, task):
    """
    Extract the program after "```python", (before "```" if applicable).
    """
    if task != 'cot':
        # since the prompt contains the ```python, model should not add it.
        reasoning = "```python\n" + reasoning
    program = "[invalid]"
    start = False
    for line in reasoning.split("\n"):
        if line.startswith("```python"):
            program = ""
            start = True
        elif line.startswith("```") and start and not line.startswith("```python") and not line.startswith("```latex"):
            # ^ cannot return early unless the program has started being collected
            start = False
            return program.strip() # only gets the first program.
        elif start:
            program += line + "\n"
    return program.strip() # a unfinished program

def extract_pdecontrol_sstl_answer(input, reasoning, task):
    """
    Extract the SSTL (Latex) answer from the reasoning.
    extracts the program after "```latex", and (before "```" if applicable).
    """
    if task != 'cot':
        # since the prompt contains the ```latex, model should not add it.
        reasoning = "```latex\n" + reasoning
    program = "[invalid]"
    start = False
    for line in reasoning.split("\n"):
...
def extract_pdecontrol_python_and_sstl(input, reasoning, task):
    """
    Extract the python and SSTL answers from the reasoning.
    """
    python = extract_pdecontrol_python_answer(input, reasoning, 'cot')
    sstl = extract_pdecontrol_sstl_answer(input, reasoning, 'cot')
    return python, sstl