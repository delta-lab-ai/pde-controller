...
class FewShotDPO(FewShotPrompting):
    def __init__(self, num_shots=0, format=None, dataset=None) -> None:
        ...
        if num_shots > 0:
            ...
            jsonl_file_path = os.path.join(current_dir, "examples", f"DPO_one_d_{dataset}", "examples.jsonl")
    ...
    def format_prompt(self, format, nl, sstl="", python=""):
        ...
        #     return prompt
        if format == "dpo_train_nl_to_sstl":
    ...
    def format_prompt_test(self, nl, sstl="", python=""):
        ...
        curr_prompt = self.format_prompt(self.format, nl.strip(), sstl.strip(), python.strip())
