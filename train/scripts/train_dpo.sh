...
### Note only specify path to base models for which an adapter will be attached.
base_model="$MODELS/MathCoder2-DeepSeekMath-7B" # Base model for the translator
translator_adapter="outputs/translator/checkpoint-step-3000" # adapter attached to translator base model
...
datafile_name=train/not-grouped_MaxContext4096_dpo_tokenized_train.parquet
...
train() {
  ...
  cmd="CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES OMP_NUM_THREADS=1 torchrun --nnodes $WORLD_SIZE --node_rank $RANK --nproc_per_node $GPUPerNode --rdzv-backend=c10d --rdzv-endpoint=localhost:0 train/train_dpo.py \
  --ddp_timeout 360000 \
  ...
  --lora_alpha 256 \
  --lora_dropout 0.1 \
  --adapter_cfg $translator_adapter \
...
for ((r=0; r<iterations; r++)); do
  ...
  exit_status=$?
  validate $CHECKPOINT_DIR/checkpoint-step-$epoch_size/policy
done
